...
// http://adminer.test, downloading the single-file app on first use. A small
...
`
	...
	if err := a.ensureToolProject("adminer", "adminer.test", dir); err != nil {
		return "", fmt.Errorf("registering adminer.test: %w", err)
	}
...
// local MySQL as root without asking. Safe only because the phpmyadmin.test
// site is local-only.
...
// phpmyadmin.test site only accepts requests from this machine.
