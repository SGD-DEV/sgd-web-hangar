...
func (a *App) OpenAdminer() (string, error) {
	...
	if err := writeAdminerIndex(dir); err != nil {
		return "", err
	}
...
// adminerIndex wraps Adminer so it logs in by itself: Hangar's local MySQL
// root / PostgreSQL postgres accounts have no password (Adminer refuses
// empty passwords by default), and the site only answers requests from this
// machine (Require local). Without a session it signs in to MySQL, or to
// PostgreSQL with ?hangar=pgsql.
const adminerIndex = `<?php
// Generated by Hangar - rewritten on every start, don't edit.
if (empty($_POST) && !isset($_GET['username'])) {
    $pg = isset($_GET['hangar']) && $_GET['hangar'] === 'pgsql';
    $_POST['auth'] = array(
        'driver' => $pg ? 'pgsql' : 'server',
        'server' => '127.0.0.1',
...
func writeAdminerIndex(dir string) error {
	return os.WriteFile(filepath.Join(dir, "index.php"), []byte(adminerIndex), 0644)
}

// refreshAdminerIndex updates the wrapper of an existing Adminer install,
// so improvements reach it without opening Adminer from Hangar first.
func (a *App) refreshAdminerIndex() {
	dir := filepath.Join(a.paths.DataPath(), "tools", "adminer")
	if _, err := os.Stat(filepath.Join(dir, "adminer.php")); err == nil {
		_ = writeAdminerIndex(dir)
	}
}
